...
def search_img(refFilename):
    ...
    if PI:
        ...
        while True:
            ...
            t2 = time.time()
            found, dst_points = match_images(imReference, frame)
            t3 = time.time()
            print("time to match %.2f" % (t3 - t2))
        ...
        while True:
            ret_val, img = cam.read()
            found, dst_points = match_images(imReference, img)

            return found, dst_points


def get_center(dst_pts):
    total_x_sum = 0
    total_y_sum = 0
